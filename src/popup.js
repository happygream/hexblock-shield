...
function bindSettings() {
  const gwUrl     = document.getElementById('gw-url');
  const sbApi     = document.getElementById('sb-api');
  const gwTestBtn = document.getElementById('gw-test-btn');
  const gwDot     = document.getElementById('gw-dot');
  const gwStatus  = document.getElementById('gw-status');

  function setGwStatus(state, text) {
    const colours = { ok: '#00e8c0', fail: '#ff4d6d', testing: '#ffbb33', idle: 'var(--line)' };
    if (gwDot)    gwDot.style.background = colours[state] || colours.idle;
    if (gwStatus) {
      gwStatus.textContent = text;
      gwStatus.style.color = state === 'ok'   ? '#00e8c0' :
                             state === 'fail'  ? '#ff4d6d' : 'var(--muted)';
    }
  }

  // Show idle status if URL is saved
  if (settings.gatewayUrl && gwUrl) {
    gwUrl.value = settings.gatewayUrl;
    setGwStatus('idle', 'NOT TESTED');
  }

  gwTestBtn?.addEventListener('click', async () => {
    const url = gwUrl?.value?.trim();
    if (!url) { setGwStatus('fail', 'ENTER A URL FIRST'); return; }

    setGwStatus('testing', 'TESTING...');
    gwTestBtn.disabled    = true;
    gwTestBtn.textContent = '...';
    ...
    try {
      ...
      if (res.ok) {
        setGwStatus('ok', 'CONNECTED');
        await msg({ type: 'SET_SETTINGS', settings: { gatewayUrl: url } });
      } else {
        setGwStatus('fail', `ERROR ${res.status}`);
      }
    } catch(e) {
      const reason = e.name === 'TimeoutError' ? 'TIMEOUT'     :
                     e.name === 'TypeError'    ? 'UNREACHABLE' : 'FAILED';
      setGwStatus('fail', reason);
    } finally {
      gwTestBtn.disabled    = false;
      gwTestBtn.textContent = 'Test';
    }
  });
